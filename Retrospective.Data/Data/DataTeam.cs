...
namespace Retrospective.Data
{
  public class DataTeam : IDataTeam
  {
    ...
    /// <summary>
    /// Get the teams that the email address has been invited to
    /// </summary>
    /// <param name="email"></param>
    /// <returns></returns>
    public List<Team> GetTeamInvitations(string email)
    {
      var teams = database.MongoDatabase.GetCollection<Team>(collection);
      var query = teams.AsQueryable<Team>()
          .Where(t => t.Invited.Any(m => m.Email == email))
          .Select(t => t);
      return query.ToList<Team>();
    }
    ...
    public List<Team> GetOwnedTeams(string userId)
    {
      return this.GetOwnedTeams(new ObjectId(userId));
    }
